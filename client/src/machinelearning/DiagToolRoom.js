...
export const startVoiceRecognitionHandler = (userRole, socket, roomID) => {
    timedRecognition(window.recognizer, window.classLabels, window.chart, window.labelContainer, userRole, socket, roomID);
};


async function timedRecognition(recognizer, classLabels, chart, labelContainer, userRole, socket, roomID) {
    ...
    recognizer.listen(result => {
        ...
        if (top3Scores[0].score >= threshold || Date.now() - startTime >= recognitionDuration) {
            ...
            outputTopResults(bestScores, userRole, socket, roomID);
        }
    }, {
...
export function outputTopResults(topScores, userRole, socket, roomID) {
    if (topScores.length === 0) return;

     const outputDiv = document.getElementById("output");
     const commentDiv = document.getElementById("comment");

    // Construct the output HTML text
    let outputText = "<strong>Recognized Sounds:</strong><br>";
    topScores.forEach((score, index) => {
        outputText += `${index + 1}st: ${score.label} - ${score.score.toFixed(2)}<br>`;
    });

    // Send results to the clinician if the user is a patient
    if (userRole === "patientslp" && socket && socket.readyState === WebSocket.OPEN) {
        console.log("Sending output to WebSocket: ", outputText); // Log the output before sending
        const message = {
    ...
    //     };
    //     socket.send(JSON.stringify(message));
    //     outputDiv.innerHTML = "Great! Voice recognized! Wait for your clinician.";
...
function assignScore(topScores) {
    ...
    scoreOutputDiv.innerHTML = `<div><strong>Recognition Score:</strong> ${score}</div>`;
}
