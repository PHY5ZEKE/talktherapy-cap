...
export default function Profile() {
  ...
  const handleShowModal = () => {
    ...
    setMiddleName(adminData.middleName);
    setLastName(adminData.lastName);
    setAddress(adminData.clinicAddress);
    setShowModal(true);
  };
  ...
  const handleEditProfileSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8000/adminSLP/edit-admin",
        {
          method: "PUT",
          headers: {
  ...
  const handleProfilePictureUpload = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8000/adminSLP/update-profile-picture",
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleChangePasswordSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8000/adminSLP/change-password",
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
